...
Each demo dataset is fully parameterizable. Tune sample size, time horizon, or random seed, then inspect the ground-truth attribution shares the DGP planted:

```python
from treemmm.demo.datasets.pharma_brand import generate_pharma_dataset

# Headline scale: 3,000 HCPs x 36 months. Defaults to 500 x 24 for quick tests.
ds = generate_pharma_dataset(n_customers=3000, n_periods=36, random_state=42)

# The reference attribution every model is benchmarked against:
print(ds.ground_truth.attribution_shares)
# {'rep_visits': 0.38, 'dtc_advertising': 0.20, 'samples': 0.32, ...}

# The raw panel: one row per (HCP, month):
print(ds.df.head())

# Column roles (which column is the customer ID, time index, outcome, promo, control):
print(ds.columns)
```

The other DGPs follow the same pattern: `from treemmm.demo.datasets.cpg_brand import generate_cpg_dataset` (and similarly for `saas_brand`, `linear_baseline`, `pharma_adstock`, `geo_panel`).

### Reproducing the headline numbers

The full multi-seed benchmark (4 DGPs x 5 seeds x 6 baselines) underlying the v0.2.1 headline is driven by `paper/run_multiseed.py`. To reproduce:

```bash
# Single-seed end-to-end on a smaller scale (~5 min on a laptop):
...
python paper/dump_per_customer_shap.py      # per-customer attribution dump
```
