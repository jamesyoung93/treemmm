...
def main() -> None:
    ...
    print(f"  Outcome: {outcome_col} (mean={df[outcome_col].mean():.1f})")
    ...
    # Attribution shares (fraction of total outcome explained by each variable).
    # Includes the base/intercept under the key "_base"; sums to ~1.0.
    ...
    for fold in result.fold_metrics:
        print(
            f"  Fold {fold['fold']}: "
            f"R2={fold['r2']:.3f}  WMAPE={fold['wmape']:.1%}  "
            f"MAE={fold['mae']:.2f}  (n_test={fold['n_test']})"
        )
