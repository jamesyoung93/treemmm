...
\usepackage{rotating}          % landscape table floats
...
\renewcommand{\UrlFont}{\fontsize{9}{11}\selectfont\ttfamily}
...
%% AUTHOR-INPUT REQUIRED: replace the GitHub noreply address with the corresponding-author email.
\ead{46870524+jamesyoung93@users.noreply.github.com}
\cortext[cor1]{Corresponding author.}
%% AUTHOR-INPUT REQUIRED: confirm the complete affiliation and country string.
...
\cite{ke2017lightgbm,chen2016xgboost,prokhorenkova2018catboost} and SHAP attribution to
...
decisions across the marketing portfolio \cite{little1979aggregate}.

\subsection{Existing Tools}
\label{sec:intro:tools}

The open-source MMM ecosystem is dominated by Bayesian and ridge regression
approaches \cite{meridian2024google,pymcmarketing2023}
(\Cref{tab:existing_tools}).
...
\item \textbf{Link-function-aware attribution}: SHAP TreeExplainer
  \cite{lundberg2017unified} computes
...
saturation parameters \cite{jin2017bayesian,chan2017challenges}, priors
regularize estimation and allow transfer of prior
...
\lambda_{i,t}
  &= \exp\!\left(
  \begin{aligned}
  &\beta_0 + \sum_k \beta_{i,k} f_k(x_{i,k,t}) \\
  &\quad + \gamma_1 x_{\text{rep},i,t} x_{\text{samp},i,t}
          + \gamma_2 x_{\text{dtc},i,t} x_{\text{rep},i,t} \\
  &\quad + \gamma_3 x_{\text{peer},i,t} x_{\text{rep},i,t}
          + \delta Z_{i,t}
  \end{aligned}
  \right), \\
y_{i,t}
  &\sim \text{NegBin}(\lambda_{i,t},\, r{=}5).
\end{split}
\end{equation}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=0.90\linewidth,height=0.78\textheight,keepaspectratio]{figures/fig12_calibration_deciles.pdf}
...
\begin{figure}[ht]
  ...
  \caption{Power analysis: Attribution MAPE vs.\ number of customers
           ...
           (The full sweep is deferred to future work because of computational
           cost; the $n{=}3000$ column is populated.)}
  \label{fig:power}
\end{figure}
...
\begin{tabularx}{\linewidth}{@{}lLccL@{}}
\toprule
...
\textbf{Method.} The
\path{treemmm.mroi.reallocate(model,X,budget_delta_pct,cap_percentile,channel)}
routine raises a channel's
...
(\ref{sec:methods:mroi}). A real deployment should still run multivariate
...
\begin{tabularx}{\linewidth}{@{}LLLCCC@{}}
\toprule
Dataset & Worst-channel swing & Worst channel & Divergences & min ESS\_bulk & max $\hat{R}$ \\
\midrule
Pharma (NegBin)  & 0.0007 (0.07 pp) & \nolinkurl{samples}     & 0 & 603   & 1.010 \\
CPG (Tweedie)    & 0.0001 (0.01 pp) & \nolinkurl{digital_spend} & 0 & 2,203 & 1.010 \\
SaaS (ZI-Gamma)  & 0.001  (0.1 pp)  & \nolinkurl{sdr_outreach}  & 0 & 3,312 & 1.010 \\
Linear (Gaussian)& 0.0001 (0.01 pp) & \nolinkurl{channel_c}     & 0 & 9,282 & 1.000 \\
\bottomrule
\end{tabularx}
\end{table}
...
adstock decay \cite{broadbent1979oneway} = 0.50 on
\texttt{rep\_visits}) to benchmark adstock-aware
...
Results are from a small-scale run (N = 200 $\times$ 18 periods); a full-scale
rerun (N = 3,000 $\times$ 36) is deferred to future work because of
computational cost.
...
The pharma column shows a single-seed crossover at $n{=}500$
...
The author also used Anthropic Claude for review assistance.
...
\textbf{PyMC-Marketing} (v0.19.4, \texttt{pymc-labs/pymc-marketing}): The leading
open-source Bayesian MMM \cite{pymcmarketing2023}, implementing NUTS sampling with geometric adstock and
...
extracted via \path{compute_channel_contribution_original_scale()}.
